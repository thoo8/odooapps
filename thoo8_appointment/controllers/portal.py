...
class AppointmentPortal(http.Controller):
    ...
    def submit_appointment(self, **post):
        ...
        required_fields = ['beneficiary_name', 'id_card', 'mobile', 'service_type', 'otp']
        missing = [field for field in required_fields if not post.get(field)]

        if missing:
            return request.render("website.404", {'error': 'Missing fields: ' + ', '.join(missing)})
        ...
        # تحقق رقم الهوية
        if not id_card or not re.match(r'^[12]\d{9}$', id_card):
            return request.render('thoo8_appointment.portal_errore_submit_template', {
                'error_message': _("Invalid ID Card number. It must start with 1 or 2 and contain 10 digits."),
            })
    ...
    def send_sms_via_msegat(self, mobile, message):
        ...
        values = '''{
                          ...
                          "numbers": " ''' + mobile + '''  ",
                          "userSender": "NAFA",
                          "apiKey": "d7abdfe7ccc6a27becc6cff91a0b519c",
                          "msg":    " ''' + message + '''  "
                        }'''
    ...
    def validate_otp_session(self, otp_code, mobile):
        session_code = request.session.get('otp_code')
        session_mobile = request.session.get('otp_mobile')
