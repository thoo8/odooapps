...
class Appointment(models.Model):
    ...
    _sql_constraints = [
        ...
        ('check_id_card', 'check(LENGTH(id_card)) <> 10)', "(Id id_card_test Must To Be 10 digit!)")]
    ...
    def create(self, vals):
        ...
        rec.template_sms_resulte('create', send_sms)
    ...
    def template_sms_resulte(self, status, response_list):
        ...
        sms_mobile = response['mobile'] or ''
        sms_text = response['message_text'] or ''
        ...
        for rec in self:
            ...
            if rec.id_card:
                if not re.fullmatch(r'\d{10}', rec.id_card):
                    raise ValidationError(_("رقم الهوية يجب أن يتكون من 10 أرقام."))

            # التحقق من رقم الجوال: 10 أرقام ويبدأ بـ 05
            if rec.mobile:
                if not re.fullmatch(r'05\d{8}', rec.mobile):
                    raise ValidationError(_("رقم الجوال يجب أن يتكون من 10 أرقام ويبدأ بـ 05."))
